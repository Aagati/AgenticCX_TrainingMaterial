# -*- coding: utf-8 -*-
"""
CAPSTONE - Telecom: Multi-Agent Team over MCP (REFERENCE SOLUTION)
...
async def secure_call_tool(session: ClientSession, tool_name: str, args: dict, ctx: dict):
    ...
    Returns EITHER a plain dict (the call was rejected before it ever
    reached the server - zero MCP round trips) OR the raw MCP
    CallToolResult (the call actually executed). Callers must check
    isinstance(result, dict) to tell the two apart - see mcp_text() /
    mcp_json_list() for turning the CallToolResult case into something
    usable.
    """
    agent_role = ctx["agent_role"]
    allowed_tools = load_entitlements().get(agent_role, {}).get("allowed_tools", [])
    if tool_name not in allowed_tools:
        log_step("secure_call_tool:rejected", {"tool_name": tool_name, "agent_role": agent_role})
        return {"error": "tool_not_allowed_for_role", "tool_name": tool_name}

    secure_args = dict(args)
    if tool_name in _IDENTITY_SCOPED_TOOLS:
        secure_args["customer_id"] = ctx["customer_id"]
        secure_args["agent_role"] = agent_role

    if tool_name in _IDEMPOTENT_TOOLS and not secure_args.get("idempotency_key"):
        ctx["turn_counter"] += 1
        secure_args["idempotency_key"] = f"auto-{ctx['session_id']}-{ctx['turn_counter']}-{tool_name}"

    return await session.call_tool(tool_name, secure_args)
...
async def run_specialist(name: str, question: str, ctx: dict) -> str:
    ...
    allowed_tool_names = load_entitlements()[agent_role]["allowed_tools"]
    ...
    for _ in range(MAX_TOOL_TURNS):
        response = client.messages.create(
            model=MODEL, max_tokens=700, system=spec["system"], tools=tools, messages=messages,
        )
        ctx["turn_counter"] += 1
        record_usage(response, agent_role, ctx["session_id"], ctx["turn_counter"])

        tool_uses = [b for b in response.content if b.type == "tool_use"]
        if not tool_uses:
            final_text = extract_text(response)
            session_info = {"customer_id": ctx["customer_id"], "owned_accounts": ctx["owned_accounts"]}
            ok, guard_reason = output_guardrail(final_text, allowed_doc_ids, session_info)
            if not ok:
                return f"I'm not able to share that. [blocked at output: {guard_reason}]"
            return final_text

        messages.append({"role": "assistant", "content": response.content})
        results = []
        for tool_use in tool_uses:
            raw_result = await secure_call_tool(ctx["mcp_session"], tool_use.name, tool_use.input, specialist_ctx)
            if isinstance(raw_result, dict):
                # Rejected client-side - never reached the server.
                result_text = json.dumps(raw_result)
            elif tool_use.name == "search_kb":
                docs = mcp_json_list(raw_result)
                kept, _blocked = sanitize_retrieved_docs(docs)
                allowed_doc_ids.extend(d["doc_id"] for d in kept)
                result_text = json.dumps(kept)
            else:
                result_text = mcp_text(raw_result)
            results.append({"type": "tool_result", "tool_use_id": tool_use.id, "content": result_text})
        messages.append({"role": "user", "content": results})

    return "(specialist hit the tool-call limit without producing a final reply)"
...
async def execute_handoff(specialist: str, question: str, ctx: dict) -> str:
    ...
    return await run_specialist(specialist, question, ctx)
...
async def run_turn(user_message: str, ctx: dict) -> str:
    ...
    _set_trace_session(ctx["session_id"])
    messages = [{"role": "user", "content": user_message}]

    for _ in range(MAX_TOOL_TURNS):
        response = client.messages.create(
            model=MODEL, max_tokens=500, system=SUPERVISOR_SYSTEM, tools=[HANDOFF_TOOL], messages=messages,
        )
        ctx["turn_counter"] += 1
        record_usage(response, "supervisor", ctx["session_id"], ctx["turn_counter"])
        ...
        messages.append({"role": "assistant", "content": response.content})
        specialist_reply = await execute_handoff(tool_use.input["specialist"], tool_use.input["task"], ctx)
        messages.append({"role": "user", "content": [
            {"type": "tool_result", "tool_use_id": tool_use.id, "content": specialist_reply}
        ]})

    return "(supervisor hit the tool-call limit without producing a final reply)"
