# -*- coding: utf-8 -*-
"""
CAPSTONE - Telecom: Full MCP Server (REFERENCE SOLUTION)

See starter/mcp_server.py for the full walkthrough. Summary of what's below:
...
def _idempotent(tool_name: str, idempotency_key: str, args: dict, compute) -> dict:
    """Given: three-branch idempotency core, extending Day4 PM_H1's
    single-branch PROCESSED_KEYS check with real 409-conflict semantics.

      - key never seen                          -> run compute(), store
        {args, result}, audit_log(tool_name)
      - key seen, SAME args as last time         -> return the stored
        result verbatim, no mutation, audit_log(f"{tool_name}_replay")
      - key seen, DIFFERENT args than last time  -> {"status": "conflict"},
        no mutation, audit_log(f"{tool_name}_conflict")
    ...
    key = f"{tool_name}:{idempotency_key}"
    if key in PROCESSED_KEYS:
        stored = PROCESSED_KEYS[key]
        if stored["args"] == args:
            audit_log(f"{tool_name}_replay", "agent", {"idempotency_key": idempotency_key, "args": args})
            return stored["result"]
        result = {"status": "conflict", "reason": "idempotency_key reused with different arguments"}
        audit_log(f"{tool_name}_conflict", "agent",
                  {"idempotency_key": idempotency_key, "args": args, "stored_args": stored["args"]})
        return result

    result = compute()
    PROCESSED_KEYS[key] = {"args": args, "result": result}
    audit_log(tool_name, "agent", {"idempotency_key": idempotency_key, "args": args, "result": result})
    return result
...
def apply_billing_credit(account_id: str, customer_id: str, agent_role: str,
    ...
    """Apply a goodwill billing credit to an account. Gated by
    check_permission (ownership + role capability + the role's
    max_credit_cents limit), then idempotent, then audited - same shape
    as create_service_ticket above.
    ...
    """
    allowed, reason_denied = check_permission(
        customer_id, account_id, agent_role, "apply_billing_credit", amount_cents=amount_cents)
    if not allowed:
        audit_log("apply_billing_credit_denied", agent_role,
                   {"account_id": account_id, "amount_cents": amount_cents, "reason": reason_denied})
        return {"error": reason_denied}

    args = {"account_id": account_id, "amount_cents": amount_cents, "reason": reason}

    def compute():
        account = _ACCOUNTS_DATA["accounts"][account_id]
        account["balance_cents"] -= amount_cents
        return {
            "status": "applied",
            "credit_id": f"CR-{uuid.uuid4().hex[:6].upper()}",
            "new_balance_cents": account["balance_cents"],
        }

    result = _idempotent("apply_billing_credit", idempotency_key, args, compute)
    log(f"apply_billing_credit -> {result}")
    return result
...
def change_plan(account_id: str, customer_id: str, agent_role: str,
    ...
    """
    allowed, reason_denied = check_permission(customer_id, account_id, agent_role, "change_plan")
    if not allowed:
        audit_log("change_plan_denied", agent_role,
                   {"account_id": account_id, "new_plan_id": new_plan_id, "reason": reason_denied})
        return {"error": reason_denied}

    account = _ACCOUNTS_DATA["accounts"][account_id]
    is_downgrade = PLAN_TIERS.get(new_plan_id, 0) < PLAN_TIERS.get(account["plan_id"], 0)
    can_override = load_entitlements().get(agent_role, {}).get("limits", {}).get("can_override_contract", False)
    if is_downgrade and account.get("contract_end") and not can_override:
        audit_log("change_plan_denied", agent_role,
                  {"account_id": account_id, "new_plan_id": new_plan_id, "reason": "in_contract_downgrade_requires_override"})
        return {"error": "in_contract_downgrade_requires_override"}

    args = {"account_id": account_id, "new_plan_id": new_plan_id}

    def compute():
        account["plan_id"] = new_plan_id
        return {"status": "applied", "account_id": account_id, "new_plan_id": new_plan_id}

    result = _idempotent("change_plan", idempotency_key, args, compute)
    log(f"change_plan -> {result}")
    return result
