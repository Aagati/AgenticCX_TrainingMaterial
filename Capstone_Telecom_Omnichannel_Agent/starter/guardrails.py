# -*- coding: utf-8 -*-
"""
Part 5 - Defence-in-Depth Guardrails
...
def layer_detect_instruction_injection(text: str) -> dict:
    ...
    Given: INSTRUCTION_INJECTION_PATTERNS below is the pattern list -
    you don't need to write the regexes, just apply them.

    Steps:
      1. Lowercase the text.
      2. Check it against each pattern in INSTRUCTION_INJECTION_PATTERNS
         with re.search.
      3. Return {"flagged": True, "reason": <the matched pattern>} on
         the first match; if none match, return
         {"flagged": False, "reason": None}.
    """
    raise NotImplementedError("TODO: implement layer_detect_instruction_injection()")
...
def sanitize_retrieved_docs(docs: list[dict]) -> tuple[list[dict], list[str]]:
    ...
    Steps:
      1. For each doc in docs, call run_layers() against
         f"{doc['title']} {doc['text']}".
      2. If it passes (not flagged), keep the doc.
      3. If it's flagged, add its doc_id to a blocked list instead of
         keeping it, and log_step("sanitize:blocked",
         {"doc_id": ..., "layer": <the failed layer name>}).
      4. Return (kept_docs, blocked_doc_ids).
    """
    raise NotImplementedError("TODO: implement sanitize_retrieved_docs()")
...
def output_guardrail(answer: str, allowed_doc_ids: list[str], session: dict) -> tuple[bool, str | None]:
    ...
    Given: the three regex constants above - you don't need to write
    the patterns, just apply them.

    Steps:
      1. Find every TEL-xxx-## doc-id-shaped token in answer using
         _DOC_ID_PATTERN.findall(). If any is NOT in allowed_doc_ids,
         return (False, "hallucinated_citation") - the reply cited a
         document that was never actually retrieved.
      2. Find every ACC-##### account-id-shaped token in answer using
         _ACCOUNT_ID_PATTERN.findall(). If any is not in
         session["owned_accounts"], return (False, "account_leak") -
         the reply mentions an account this customer doesn't own.
      3. Search answer for _IDEMPOTENCY_KEY_PATTERN. If found, return
         (False, "internal_key_leak") - idempotency keys are
         implementation detail, never customer-facing.
      4. Otherwise return (True, None).
    """
