"""
AM · H1b — Banking: Use an MCP Server (STARTER)

An agent tool-use loop where the tools don't live in this process — they
live in a separate process (server_starter.py, Part A) and are called
over the real MCP protocol: this process spawns the server over stdio,
discovers its tools via session.list_tools() (no hardcoded schema — the
server is the only source of truth), and executes them via
session.call_tool().
...
def mcp_tool_to_anthropic_schema(mcp_tool) -> dict:
    ...
    hand-maintained CREATE_TICKET_TOOL dict like a plain in-process tool."""
...
async def run_turn(session: ClientSession, tools: list, messages: list) -> list:
    ...
    # TODO 2: The actual call crosses process boundaries over MCP's stdio
...
async def main():
    # TODO 3: Build a StdioServerParameters(command=sys.executable,
    # args=[str(SERVER_SCRIPT)]), then open `async with stdio_client(server_params)
...
# process over stdio. Kill server_starter.py's process independently,
