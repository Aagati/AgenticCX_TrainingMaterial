"""
PM · H1b — Banking: Idempotent, Audited Refund Client (REFERENCE SOLUTION)

The MCP mechanics here (spawn the server, discover its tools, call them
over the protocol) are the same ones built in AM_H1b — given as working
boilerplate so the graded effort goes into the ONE new piece: simulating a
network retry by calling process_refund a SECOND time, directly, with the
exact same idempotency_key the model used the first time, and confirming
the server dedupes it instead of refunding twice.
