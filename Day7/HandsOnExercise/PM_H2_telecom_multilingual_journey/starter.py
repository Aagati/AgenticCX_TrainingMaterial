"""
PM · H2 — Telecom: Multilingual Journeys, Personas & Hand-off (STARTER)

A "journey" spans more than one touch — a chat today, a voice call
tomorrow. If the agent starts from zero on every touch, the customer
re-explains their problem every time. This lab makes journey CONTINUITY a
real object (JourneyMemoryStore) instead of a concept: a fact extracted
from a CHAT turn must be available to a VOICE turn that happens later,
keyed on customer_id, not on channel or session.

You'll build:
  1. JourneyOrchestrator.advance_turn — a real Claude call per turn that
     folds in locale persona + accumulated memory and returns a forced-
     schema decision (stage/reply/fact).
  2. HandoffPackager.build_handoff — a real Claude call that turns the
     full multilingual journey into an ENGLISH bundle a human agent (who
     may not speak the customer's language) can act on immediately.
"""
...
class JourneyOrchestrator:
    ...
    def advance_turn(self, customer_id: str, channel: str, locale: str, utterance: str) -> TurnResult:
        """
        TODO 1: Make a real sonnet call, forced tool use:
          - persona = LanguageRouter.route(locale)
          - known_facts = self.memory.get_facts(customer_id)
          - system prompt: a telecom support agent, respond in
            persona['language_name'], use persona['tone']; explain this is
            a multi-touch journey and the customer may have contacted
            before on a DIFFERENT channel; include known_facts and instruct
            the model to use them naturally instead of making the customer
            repeat themselves; explain the three stage values and when to
            pick each; instruct it to capture any new durable fact in
            `fact`, else leave it null
          - user content: f"[channel: {channel}] {utterance}"
          - client.messages.create(model=MODEL, max_tokens=1024,
            system=..., messages=[{"role": "user", "content": user}],
            tools=[{"name": "advance", "description": ...,
            "input_schema": TurnResult.model_json_schema()}],
            tool_choice={"type": "tool", "name": "advance"})
          - parse via next(b for b in response.content if b.type ==
            "tool_use"), build TurnResult(**tool_call.input)
          - IMPORTANT: if result.fact is truthy, call
            self.memory.add_fact(customer_id, result.fact) before returning
        """
...
class HandoffPackager:
    ...
    def build_handoff(customer_id: str, locale: str, transcript: list[str], memory: JourneyMemoryStore) -> HandoffSummary:
        """
        TODO 2: Make a real sonnet call, forced tool use:
          - facts = memory.get_facts(customer_id)
          - system prompt: summarize a multilingual support journey for a
            human agent taking over; write summary/key_facts/
            recommended_action in ENGLISH regardless of what language the
            customer spoke, since the receiving agent may not read it
          - user content: include locale, facts, and "\n".join(transcript)
          - client.messages.create(model=MODEL, max_tokens=1024,
            system=..., messages=[{"role": "user", "content": user}],
            tools=[{"name": "summarize_handoff", "description": ...,
            "input_schema": HandoffSummary.model_json_schema()}],
            tool_choice={"type": "tool", "name": "summarize_handoff"})
          - parse the tool_use block and return HandoffSummary(**tool_call.input)
        """
        raise NotImplementedError
...
if __name__ == "__main__":
    ...
    transcript.append("[chat] Customer: My router keeps disconnecting every evening around 8pm, three nights in a row now.")
