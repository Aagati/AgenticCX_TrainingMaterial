"""
Capstone: Banking - Enterprise Won't Trust a Demo (SOLUTION).
...
capstone in this curriculum did:
  - Day 4's idempotent, audited action (`process_refund` /
    ...
    retried under a circuit breaker (`guarded_action`). 20-ish physical
    attempts collapse to a handful of ledger rows - idempotency and retry
...
You'll build:
  ...
  7. run_corpus, BatchGate.build_requests/.score, eval_gated.
"""
...
def guard_rbac_ceiling(payload: str, context: dict) -> GuardrailResult:
    """Generalizes Day 4's per-user ownership check into role ->
    capability, with a numeric ceiling Day 4 had no analogue for."""
    if context["action"] == "issue_provisional_credit" and context["credit_amount"] is not None:
        ceiling = context["role"]["max_credit_authority"]
        if context["credit_amount"] > ceiling:
            return {"verdict": "BLOCK", "detail": f"credit {context['credit_amount']} exceeds role ceiling {ceiling}"}
    return {"verdict": "PASS", "detail": "ok"}
...
def guard_disclosure(payload: str, context: dict) -> GuardrailResult:
    ...
    if context["credit_amount"] is not None and context["credit_amount"] > jurisdiction["requires_written_confirmation_above"]:
        additions.append(WRITTEN_CONFIRMATION_SENTENCE)
    if not additions:
        return {"verdict": "PASS", "detail": "ok"}
    return {"verdict": "REDACT", "detail": f"appended {len(additions)} sentence(s)", "payload": payload.rstrip() + " " + " ".join(additions)}
...
class GuardrailStack:
    ...
    def evaluate(payload: str, context: dict) -> dict:
        ...
        for layer in ordered:
            if layer["always_runs"] or layer["group"] not in context["run_groups"]:
                continue
    ...
    def allow_request(self, now: float) -> bool:
        ...
        if self.state == "open":
            if now - self.opened_at >= self.recovery_seconds:
                ...
                return True
            return False
        return True  # half_open: the one probe in flight
...
def call_with_retry(fn, max_attempts: int = RETRY_MAX_ATTEMPTS, base_delay: float = RETRY_BASE_DELAY) -> tuple[bool, object, int]:
    """Same shape as Lab-1's ResilientCaller.call_with_retry - exponential
    backoff with full jitter, never sleeping after the last attempt."""
...
def guarded_action(dispute_id: str, amount: float, now: float, breaker: CircuitBreaker, api: CoreBankingAPI) -> dict:
    """Breaker OUTSIDE, retry INSIDE - identical composition to Lab-1's
    guarded_downstream_call."""
    if not breaker.allow_request(now):
        return {"status": "short_circuited", "attempts": 0}
    succeeded, result, attempts = call_with_retry(lambda: api.issue_provisional_credit(dispute_id, amount, idempotency_key=dispute_id))
    if succeeded:
...
def draft_response(customer: dict, context: dict, action_result: dict) -> str:
    """Grounded strictly in action_result - never claims a credit was
    issued unless it actually was."""
    first_name = customer["name"].split()[0]
    disclosure = context["jurisdiction"]["required_disclosure"]
    if action_result["status"] == "credited":
        fact = f"a provisional credit of ${action_result['amount']} has been issued to your account"
    else:
        fact = "your dispute is still being processed and no credit has been issued yet"
    system = (
        "You write short banking dispute-response messages. In 2-3 sentences: address the customer by "
        "first name, state only the fact given (never invent a different outcome), and end with the "
        "jurisdiction's required disclosure, verbatim, as the final sentence."
    )
...
def agent_node(state: CycleState) -> dict:
    ...
    now = FAULTS[customer["dispute_id"]]["t"]
    action_result = guarded_action(customer["dispute_id"], customer["dispute_amount"], now, BREAKER, CORE_BANKING_API)
    draft = draft_response(customer, context, action_result)
    return {"action_result": action_result, "draft": draft}


def guardrail_node(state: CycleState) -> dict:
    context = state["context"]
    context["run_groups"] = {"compliance"}
    result = GuardrailStack.evaluate(state["draft"], context)
    return {"compliance_trail": result["trail"], "passed": result["final_verdict"] == "PASS",
...
class EnterpriseCommandCenter:
    ...
    def _build_graph(self):
        ...
        graph.add_conditional_edges("intake", _route_after_intake, {"proceed": "agent", "deny": "deny"})
        graph.add_edge("agent", "guardrail")
        graph.add_conditional_edges("guardrail", _route_after_guardrail,
                                     {"release": "release", "handoff": "handoff", "repair": "repair", "deny": "deny"})
        graph.add_edge("repair", "guardrail")
        graph.add_edge("release", END)
        graph.add_edge("handoff", END)
        graph.add_edge("deny", "handoff")
...
class CapacityMeter:
    ...
    def compute(records: list[dict]) -> dict:
        """Day 5's ROIResult floor-gate-first idea, recast: don't approve a
        rollout on cost/volume numbers alone if the safety floor itself is
        failing."""
        from collections import Counter
        volume_by_jurisdiction = dict(Counter(r["jurisdiction"] for r in records))
        outcome_counts = dict(Counter(r["outcome"] for r in records))
        n = len(records)
        pass_rate = round(sum(1 for r in records if r["first_pass_passed"] is True) / n, 3) if n else 0.0
        approved = pass_rate >= 0.5
        return {
            "total_processed": n, "volume_by_jurisdiction": volume_by_jurisdiction,
            "outcome_counts": outcome_counts, "guardrail_pass_rate": pass_rate,
            "approved": approved, "rejection_reason": None if approved else "guardrail_pass_rate below the 0.5 floor",
        }
...
def run_corpus(traces: list[dict]) -> dict:
    """Same two-sided scoring shape as Lab-2's run_corpus - the corpus
    tests the GUARDRAIL STACK, not the action layer, so a passing trace's
    payload is treated as already-credited for grading purposes."""
    missed_blocks, false_blocks, wrong_layer, redaction_mismatches = [], [], [], []
    blocked = 0
    results = []
    for trace in traces:
        customer = CUSTOMERS_BY_ID[trace["customer_id"]]
        context = resolve_context(customer, trace["action"], trace["credit_amount"], actor_id=trace["actor_id"])
        context["run_groups"] = {"input", "permission"}
        pre = GuardrailStack.evaluate(trace["message"], context)
        if pre["final_verdict"] == "BLOCK":
            final_verdict, blocking_layer, redactions_applied = "BLOCK", pre["blocking_layer"], []
        else:
            payload = trace["precomposed_response"] or draft_response(
                customer, context, {"status": "credited", "amount": trace["credit_amount"]})
            context["run_groups"] = {"compliance"}
            post = GuardrailStack.evaluate(payload, context)
            final_verdict, blocking_layer, redactions_applied = post["final_verdict"], post["blocking_layer"], post["redactions_applied"]

        result = {"trace_id": trace["trace_id"], "final_verdict": final_verdict, "blocking_layer": blocking_layer}
        results.append(result)
        if final_verdict == "BLOCK":
            blocked += 1
        if trace["is_adversarial"] and final_verdict != "BLOCK":
            missed_blocks.append(trace["trace_id"])
        if not trace["is_adversarial"] and final_verdict == "BLOCK":
            false_blocks.append(trace["trace_id"])
        if trace["expected_blocking_layer"] is not None and blocking_layer != trace["expected_blocking_layer"]:
            wrong_layer.append((trace["trace_id"], trace["expected_blocking_layer"], blocking_layer))
        if trace["expected_redaction_layers"] is not None and set(redactions_applied) != set(trace["expected_redaction_layers"]):
            redaction_mismatches.append((trace["trace_id"], trace["expected_redaction_layers"], redactions_applied))

    return {
        "total": len(traces), "blocked": blocked,
        "expected_blocked": sum(1 for t in traces if t["is_adversarial"]),
        "missed_blocks": missed_blocks, "false_blocks": false_blocks,
        "wrong_layer": wrong_layer, "redaction_mismatches": redaction_mismatches, "results": results,
    }
...
class BatchGate:
    ...
    def build_requests(records: list[dict]) -> list[dict]:
        requests = []
        for r in records:
            if not r.get("final_text"):
                continue
            requests.append({
                "custom_id": r["customer_id"],
                "params": {
                    "model": MODEL_CHEAP, "max_tokens": 200,
                    "system": ("You are a banking compliance reviewer. Judge whether this customer-facing "
                               "response is well-grounded and appropriate given its stated outcome."),
                    "tools": [{"name": "judge", "description": "Record the compliance judgment.",
                               "input_schema": ComplianceJudgment.model_json_schema()}],
                    "tool_choice": {"type": "tool", "name": "judge"},
                    "messages": [{"role": "user", "content": f"Outcome: {r['outcome']}\nResponse: {r['final_text']}"}],
                },
            })
        return requests
    ...
    def run(records: list[dict], poll_interval: float = 5.0, max_wait_seconds: float = 300.0) -> dict:
        """Same submit/poll/parse shape as Day 8 Lab-1's
        InsightBatchExtractor.run."""
        requests = BatchGate.build_requests(records)
        ...
        while True:
            ...
            if waited >= max_wait_seconds:
                print("    batch gate: giving up waiting, scoring what's available")
                break
        ...
        for item in client.messages.batches.results(batch.id):
            if item.result.type != "succeeded":
                continue
            tool_use = next(b for b in item.result.message.content if b.type == "tool_use")
            judgments[item.custom_id] = ComplianceJudgment.model_validate(tool_use.input)

        pass_rate = round(sum(1 for j in judgments.values() if j.compliant) / len(judgments), 3) if judgments else 0.0
        return {"judgments": judgments, "pass_rate": pass_rate}
